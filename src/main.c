...
static char *read_text(int argc, char *argv[], int optind) {
    if (optind < argc) {
        size_t len = 0;
        for (int i = optind; i < argc; i++) len += strlen(argv[i]) + 1;
    ...
    if (!isatty(STDIN_FILENO)) {
        ...
        size_t pos = 0;
        int ch;
        while ((ch = fgetc(stdin)) != EOF && pos < MAX_TEXT - 1) buf[pos++] = ch;
        buf[pos] = '\0';
...
int main(int argc, char *argv[]) {
    static struct option opts[] = {{"target", required_argument, 0, 't'},
    ...
    if (saved_src && lang_to_code(saved_src) && strcasecmp(saved_src, target) != 0)
